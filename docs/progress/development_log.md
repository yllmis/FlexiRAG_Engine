...
- 已完成审核模块闭环优化：
  - 审核拒绝改为结构化错误，避免通过字符串拼接判断输入/输出审核来源。
  - `chat` 路径可正确区分“输入内容未通过审核”与“输出内容未通过审核”。
  - 入库审核异常已改为 `fail-closed`，异常时不再继续写库。
  - 入库 `warn/review` 结果已接入审计打点，与问答路径的审核语义保持一致。
  - 已新增 `internal/engine/executor_test.go`，并补充 `internal/api/v1/handler_test.go` 覆盖输出审核提示和入库审核审计行为。
...
- 删除流程支持清理压测生成的 `perf-agent`：可通过管理界面逐个删除，或调用删除接口进行批量清理。
- Agent 管理页面新增批量删除能力：支持勾选卡片批量删除、全选当前列表、一键选中 `perf-agent` 并批量清理。
- Agent 管理页面已移除序号展示（如 `Agent #84`），避免压测导致 ID 跳号影响日常操作体验。
